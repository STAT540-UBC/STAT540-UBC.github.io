...
subtitle: "More limma & multiple testing"
author: Keegan Korthauer
date: "8 February 2021 <br><br><font size=4> with slide contributions from Gabriela Cohen Freue, Jenny Bryan, and Sara Mostafavi </font>"
output:
...
* Feedback posted on your initial project proposals (see the issue in your team's repository)

* Reminder that the [finalized project proposals](https://stat540-ubc.github.io/subpages/group_project_rubrics.html#proposal-5-pts) are due Feb 18

* Instructions on how to submit your deliverables have been updated for clarity on the [website](https://stat540-ubc.github.io/subpages/assignments.html#how-to-turn-in-your-work)
  - in particular pay attention to the recommended way to grab the link to the submitted files from your latest commit
  
---

# Course format feedback

```{r, out.width="620", fig.align="center", echo=FALSE, include=TRUE}
knitr::include_graphics("img/SurveyResults.png")
```

<big>

* Preference for mix of synchronous & asynchronous

* 😊 Positive feedback on interactive quiz questions / polls / chat

* 😟 Several people felt a lack of communication - let's utilize the [Discussion repo](https://github.com/STAT540-UBC/Discussion/issues) more!
...
* Usage: `lmFit(myDat, desMat)` 
  ...
  * `myDat` is a data frame, matrix with features in rows and samples in columns  (*G* genes by *N* samples), or [`ExpressionSet`](https://bioconductor.org/packages/devel/workflows/vignettes/maEndToEnd/inst/doc/MA-Workflow.html#42_Bioconductor_ExpressionSets) object
...
```
.center[Bioconductor's ExpressionSet class]
]
...
* $x_{ig,KO}$ is the dummy/indicator variable for the NrlKO group
...
ggplot(topSixGenotype, aes( x = age, y = expression, color = genotype)) + 
...
ggplot(topSixAge, aes( x = age, y = expression, color = genotype)) + 
             geom_jitter(width = 0.2, alpha = 0.7) +
             xlab("age (days)") +
             facet_wrap( ~ gene) + 
             stat_smooth(method="lm", se=FALSE, cex=0.5)
...
.smaller[
```{r}
topTable(ebFit, coef = c("genotypeNrlKO", "genotypeNrlKO:age"))[,-3]
```
]
...
.left-column[
For **small** variances, limma $\rule{1.7cm}{0.15mm}$ the estimates (fill in the blank):
...
.left-column[
For **large** variances, limma $\rule{1.7cm}{0.15mm}$ the estimates (fill in the blank):
...
df %>% ggplot(aes(y=limma, x = lm)) +
  ...
  geom_abline(intercept=0, slope=1, color="red") +
  ggtitle("p-value for genotypeNrlKO:age coef") 
```
...
1. **Adjust the p-values; keep same $\alpha$:** 
    ...
    $$p_{Bon,i}=mp_i \text{ (more technically correct: } p_{Bon,i}=min(mp_i, 1))$$ 
...
# Multiple test correction is an active area of statistical <br> research
...
* What if you're not only testing 45K genes, but also multiple tests per gene (e.g. multiple contrasts, such as several two-group comparisons)?
...
* **Bootstrap**: take repeated random samples *with replacement* from your dataset (randomly assigning covariate of interest) and compute statistic; repeat many times and use bootstrap statistics as your sampling distribution rather than a t, Normal, F, $\chi^2$, etc
