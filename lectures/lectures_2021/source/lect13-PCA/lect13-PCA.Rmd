...
$$Cov(\boldsymbol{x}_1, \boldsymbol{x}_2) = \frac{\sum_{j=1}^m (x_{1j} - \bar{x}_1)(x_{2j} - \bar{x}_2)}{m-1}$$
* Intuition: *is sample 1 far away from its mean when sample 2 is far away from its mean?*

--

* Computing $Cov(\boldsymbol{x}_i, \boldsymbol{x}_{i'})$ for every pair of samples $i$ and $i'$ (note $Cov(\boldsymbol{x}_i, \boldsymbol{x}_{i}) = 1$) gives the $n\times n$ sample **covariance matrix**

* $\frac{X^TX}{m-1}$ on *centered* data gives you the sample covariance matrix

--

* Could alternatively compute covariances between all pairs for rows (features) to get $m\times m$ covariance matrix (e.g. gene covariance matrix)

* Sample **Correlation matrix**: $\frac{X^TX}{m-1}$ on *centered* and *scaled* columns 
...
* Sample **variance** of sample $i$: $Var(\boldsymbol{x}_i) = \frac{\sum_{j=1}^m (x_{ij} - \bar{x}_i)^2}{m-1}$

* Sample **covariance** of samples $i$ and $i'$: $Cov(\boldsymbol{x}_i, \boldsymbol{x}_{i'}) = \frac{\sum_{j=1}^m (x_{ij} - \bar{x}_i)(x_{i'j} - \bar{x}_{i'})}{m-1}$

* Sample (Pearson) **correlation** of samples $i$ and $i'$: : $Cor(\boldsymbol{x}_i, \boldsymbol{x}_{i'}) = \frac{Cov(\boldsymbol{x}_i, \boldsymbol{x}_{i'})}{\sqrt{Var(\boldsymbol{x}_i)Var(\boldsymbol{x}_{i'})}}$
...
* **dot / inner product**: $\boldsymbol{x}_i \cdot \boldsymbol{x}_{i'} = \sum_{j=1}^m x_{ij}x_{i'j}$
...
* $U_{m\times n}$: left singular vectors: orthogonal basis of row space of $X$

* $V_{n \times n}$: right singular vectors: orthogonal basis of column space of $X$

* $D_{n \times n}$: diagonal matrix of eigenvalues
...
* Each column of $X$ is constructed as: $\boldsymbol{x}_i = UD\boldsymbol{\nu}_i^T = \sum_{k=1}^n \boldsymbol{u}_k d_{kk} \nu_{ki}$
  ...
  * where $\boldsymbol{\nu}_i$ is the $k^{th}$ column vector of $V^T$
...
$$X = \sum_{k=1}^n d_{kk} \boldsymbol{u}_k \boldsymbol{\nu}_{k}^T$$
...
# Visualization of data based on PC 1-2
...
# Visualization of data based on PC 2-3
