...
To run the code, you'll need to these packages:

```{r installations, eval=F}
install.packages('tidyverse')
install.packages('ComplexHeatmap')
install.packages('circlize') # for ComplexHeatmap, not sure it's actually needed.
...
The batch confound is the most concerning issue, but at least the Group is not confounded with batch. We'll come back to this.
...
# pheatmap - easier to use but no ability to control the na.color!
pheatmap(cc, color=bcols, border_color=NA, cluster_rows=F, cluster_cols=F, annotation_col=data.frame(row.names = names(d), Group = m$Group, Batch=m$SeqRun, DPC=factor(m$DPC)), main="pheatmap", fontsize=8)
