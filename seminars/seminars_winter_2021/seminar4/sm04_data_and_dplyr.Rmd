...
By adding one additional function to the unmodified code (in Part 2) above, calculate the sum of all counts in each sample and divide each expression value by that sum (hint: use mutate). 

Please do not use the modified code (in Part 2 exercise) to do this exercise. Your code would be different if you use the modified data frame (one with the transcription_length) instead of the data frame in Part 2 (one without the transcription_length).
