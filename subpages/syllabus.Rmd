...
assn <- assn %>% 
  select(content,start, style) %>%
  rename(Assignment = content, `Due Date` = start) %>%
  mutate(
    Category = ifelse(
      grepl("pink",style),
      "Individual Assignment",
    ifelse(grepl("lavender",style),
      "Group Project",
    ifelse(grepl("aqua",style),
           "Paper Critique",
    ifelse(grepl("gold",style),"Warm-Up", "Seminar"))))) %>%
  select(-style) %>% 
...
assn <- assn %>% 
  ...
  arrange(`Due Date`) %>% 
  mutate(`Due Date` = format(`Due Date`, "%a %d %B %Y"))
kable(assn %>% filter(Category == "Warm-Up") %>% select(-Category))
```
**Paper critique (5%)**
...
assn <- assn %>% 
  ...
  arrange(`Due Date`) %>% 
  mutate(`Due Date` = format(`Due Date`, "%a %d %B %Y"))
kable(assn %>% filter(Category == "Individual Assignment") %>% select(-Category))
```
