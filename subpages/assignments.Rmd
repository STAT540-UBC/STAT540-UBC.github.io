...
<font color="red">**Deadlines are all by 11:59 pm (Pacific time) on the due date, and late assignments are penalize 10% per day/partial day.** If you anticipate having trouble meeting a deadline, or need to request an extension please reach out via email.</font>
...
in a 450-650 word summary/report. The summary should include: 
...
# STAT 540 Course Work Submission Instructions

**GitHub**

You all have a private repository in STAT540-UBC organization account, i.e., the repo `zz_lastname-firstnmae_STAT540_2021`. We assume that 
...
* Please use the repository within the organization assigned to you to submit all your course work (i.e., the repo `zz_lastname-firstname_STAT540_2021`). Do not use branches or any other repositories.
  
**Set-up your private GitHub repo for course work**

  * We're talking about the repo `zz_lastname-firstnmae_STAT540_2021` now. 
...
**Markdown (output file)**
  ...
  * Note that, by default, any figures created are placed into a `figures/` subdirectory. The Markdown file links to these and, therefore, requires them to present your full report. By default, the figures are base64 encoded and *embedded* into the HTML, which, therefore, is self-contained. (Note that you can also specify `self_contained: true` in the YAML header in order to encode the embedded figures directly into the output file with no separate `figures` directory)

**Optional HTML file** 

  * In addition, you may generate an html output file
    - This can be done by opening the md file generated from the previous step and clicking 'preview' in RStudio.  
  ...
  * Same considerations apply to including figure files or a self-contained option as described above for the Markdown document
...
> This is rather specific to STAT 540 and may not necessarily reflect your workflow in the future and in other contexts.
  ...
  * Commit the output Markdown (`.md`) file and the figures stored in the `figures/` subdirectory (if not using `self_contained: true`).
  * The HTML (`.html`) file is not necessary to commit if you have specified `output: github_document` and commit your md file.
  * __Never ever__ edit the output Markdown or HTML "by hand". Only edit the R Markdown source and then regenerate the downstream products from that.
...
**How to "turn in" your work**
  ...
  * Open an issue, link to the latest commit, tag us
    ...
    - Click on "Issues", then on "New Issue". Name the issue "Mark *the deliverable title* of *your repository name*", e.g. "Mark seminar 1 of zz_lastname-firstname_STAT540_2021W2"
    - In the description of the issue, tag all TAs with the <font color="red">**@STAT540-UBC/current_tas**</font> tag, and paste the revision SHA. You can also include a link to the markdown file as well. 
